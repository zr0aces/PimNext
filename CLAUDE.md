...
| Dict | Lifetime | Role |
|---|---|---|
| `print_options` | 1800s **sliding** — `get_print_options()` refreshes `ts` on every read | temporary per-session overrides |
| `last_print_time` | swept at 1800s | 10s (`PRINT_COOLDOWN`) per-chat rate limit |
| `half_queue` | 1800s | downloaded file paths awaiting a pairing file |
...
Both paths converge on `_print_and_reply()`, which owns print → notify Home Assistant → reply → delete the temporary files. These were once two hand-copied blocks; keep them merged so a fix to one cannot miss the other.
...
Every CUPS call goes through `create_subprocess_exec` with an argument list — never a shell string. `run_cups_command()` and `print_file()` both do `kill()` + `await wait()` on timeout to reap the child. `/status`, `/jobs` and `/cancel` share `run_cups_query()`, which returns `(stdout, error_message)` with exactly one set; it owns the binary check, the `-h <server>` wiring, the timeout, and stderr truncation. `LP_BIN` / `LPSTAT_BIN` / `CANCEL_BIN` are resolved by `shutil.which` once at import.
...
`main()` computes `chat_id_filter` first — it degrades to `filters.ALL` when `ALLOWED_CHAT_IDS` is unset, which is the open-to-the-world configuration and logs a warning at startup. Then it registers: the preferences `ConversationHandler`, `/help` and `/status`, the chat-filtered commands, and the two `MessageHandler`s.

- Only `/help` and `/status` are unfiltered. **`/start` and `/preferences` are chat-filtered on purpose** — saved profiles are a capped resource (`MAX_PREFERENCES`, default 10), so an open wizard lets strangers exhaust it and lock out real users. Do not remove those filters.
